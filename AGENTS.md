...
- The unified backend product registry lives in `services/patchhive-backend/registry/products/*.toml`; product modules should declare identity, route claims, capabilities, safety boundaries, gateway targets, and module paths there instead of being hardcoded in `main.rs`.
