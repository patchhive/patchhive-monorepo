...
- The unified backend product registry lives in `services/patchhive-backend/registry/products/*.toml`; product modules should declare identity, route claims, capabilities, safety boundaries, gateway targets, health contracts, smoke-tier membership/actions/fixtures/timeouts, accepted startup-check identities, and module paths there instead of being hardcoded in control-plane code.
