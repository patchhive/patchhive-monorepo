...
- HiveCore reads the canonical `services/patchhive-backend/registry/products/*.toml` manifests at startup. Those manifests own identity, display metadata and ordering, default endpoints, safety posture, capabilities, routes, health contracts, and migration stage. Invalid, incomplete, duplicate, or mismatched registry records fail startup; do not restore a parallel hardcoded product catalog.
