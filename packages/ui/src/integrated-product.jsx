...
export function PriorityHighlights({ className = "", emptyLabel = "No active findings in this run.", items = [], label = "Prioritized findings", onOpen }) {
  return (
    <div className={className}>
      <div className={`flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}><Zap size={12} style={{ color: "var(--accent-2)" }} />{label}</div>
      <div className="mt-3 space-y-2">
        {items.slice(0, 3).map((item, index) => {
          const itemTone = tone(item.tone);
          return <button className="surface-inset flex w-full items-center gap-3 rounded-xl p-3 text-left hover:brightness-110 disabled:cursor-default" disabled={!onOpen} key={`${item.id}-${index}`} onClick={() => onOpen?.(item)} type="button"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br text-[11px] font-semibold tabular-nums ${SCORE_CLASSES[itemTone]}`}>{item.score ?? index + 1}</span><span className="min-w-0 flex-1"><span className={`block truncate text-[12px] font-medium ${V3_TEXT.strong}`}>{item.title}</span><span className={`mt-0.5 block truncate text-[10px] ${V3_TEXT.mute}`}>{item.summary || item.meta || "Open for supporting evidence"}</span></span><span className={`shrink-0 rounded-full border px-2 py-1 text-[9px] uppercase tracking-wider ${STATUS_CLASSES[itemTone]}`}>{item.status || "review"}</span></button>;
        })}
        {!items.length ? <div className={`surface-inset rounded-xl px-4 py-5 text-center text-[11px] ${V3_TEXT.mute}`}>{emptyLabel}</div> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-[1200px] px-3 sm:px-6 pt-8 pb-24 grid grid-cols-12 gap-6">
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-6"><div className="surface p-6"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>{statusLabel}</div><span className={`mt-4 inline-flex text-[10px] uppercase tracking-widest px-3 py-1.5 rounded-full border ${STATUS_CLASSES[itemTone]}`}>{item.status}</span><dl className="mt-6 space-y-3"><SideValue label="Product" value={config.name} /><SideValue label="Source" value={item.source || "product analysis"} /><SideValue label="Score" value={String(item.score ?? "—")} />{item.runId ? <SideValue label="Run" value={String(item.runId).slice(0, 8)} /> : null}{item.runCreatedAt ? <SideValue label="Recorded" value={new Date(item.runCreatedAt).toLocaleString()} /> : null}{(item.facts || []).map((fact) => <SideValue key={fact.label} label={fact.label} value={String(fact.value ?? "—")} />)}</dl></div>{links.map((link) => <a className={`surface p-5 flex items-center justify-between text-[13px] ${V3_TEXT.body}`} href={link.url} key={`${link.label}-${link.url}`} rel="noreferrer" target="_blank">{link.label || "Open source evidence"} <ExternalLink size={13} /></a>)}</aside>
      </div>
    </>
  );
}
...
export function IntegratedProductApp({ apiBase, auth, config, fetcher }) {
  ...
  const [form, setForm] = useState(() => ({
    ...config.defaultForm,
    ...(config.requiresRepo === false ? {} : { repo: localStorage.getItem(`${storagePrefix}.repo`) || config.defaultForm.repo || "" }),
  }));
  ...
  useEffect(() => {
    if (config.requiresRepo !== false) localStorage.setItem(`${storagePrefix}.repo`, form.repo || "");
  }, [config.requiresRepo, form.repo, storagePrefix]);
  useEffect(() => {
  ...
  async function run() {
    const validationError = config.validate?.(form)
      || (config.requiresRepo === false || form.repo?.trim() ? "" : "Enter a repository in owner/name format before running.");
    if (validationError) {
  ...
  const priorityItems = useMemo(() => {
    if (config.priorityItems) return config.priorityItems(items, result).slice(0, 3);
    return [...items]
      .filter((item) => ["hot", "warn"].includes(tone(item.tone)))
  ...
  const connectionLabel = config.connectionLabel?.(health) || githubStatusLabel(health, true);
  const connectionValue = config.connectionValue?.(health) || githubStatusLabel(health);
  const connectionName = config.connectionName || "GitHub";
  const targetLabel = config.targetLabel?.(result, form, overview)
    || result?.repo
    || form.repo
    || (config.requiresRepo === false ? "discovery scope" : "no repository selected");
  ...
  return (
    <ProductShell productKey={config.productKey}>
      <ProductHeader activeTab={activeTab} githubLabel={connectionLabel} icon={config.icon} onRun={run} onSignOut={auth.logout} onTabChange={setActiveTab} productName={config.name} runDisabled={running} runLabel={running ? config.runningLabel : config.runLabel} subtitle={config.subtitle} tabs={tabs} />
      <div className="mx-auto max-w-[1440px] px-3 sm:px-6 pt-6 sm:pt-10 pb-24">
        ...
        {activeTab === "checks" ? <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]"><section className="surface p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Startup evidence</div><h1 className={`font-display mt-2 text-[42px] font-semibold ${V3_TEXT.strong}`}>System checks.</h1></div><button className={`surface-inset h-9 rounded-full px-4 text-[11px] ${V3_TEXT.body}`} disabled={loading} onClick={() => refresh(false)} type="button">{loading ? "Refreshing…" : "Refresh checks"}</button></div><div className="mt-7"><StartupCheckList checks={checks} /></div></section><aside className="space-y-6"><section className="surface p-6"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Unified backend</div><div className={`mt-3 font-display text-[46px] font-semibold ${V3_TEXT.strong}`}>{health.status || "unknown"}</div><dl className="mt-6 space-y-3"><SideValue label="Database" value={health.db_ok ? "ready" : "unavailable"} /><SideValue label={connectionName} value={connectionValue} /><SideValue label="Runs" value={String(history.length)} /></dl></section>{ChecksDetails ? <div className="[&>section]:mt-0 [&>section]:grid-cols-1"><ChecksDetails checks={checks} health={health} history={history} /></div> : null}</aside></div> : null}
        {activeTab === "sources" ? <><div className="grid grid-cols-12 gap-6"><section className="surface col-span-12 lg:col-span-8 p-6 sm:p-8"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Product intake</div><h1 className={`font-display mt-2 text-[42px] font-semibold ${V3_TEXT.strong}`}>{config.formTitle}</h1><div className="mt-7 grid grid-cols-1 sm:grid-cols-2 gap-4">{formFields.map((field) => <FormField field={field} form={form} inputRef={inputRef} key={field.key} setForm={setForm} />)}</div><button disabled={running || !canRun} onClick={run} className="mt-6 h-11 px-5 rounded-full text-[12px] font-semibold text-white disabled:opacity-50" style={{ backgroundImage: "linear-gradient(90deg, var(--accent), var(--accent-2))" }} type="button">{running ? config.runningLabel : config.runLabel}</button></section><aside className="surface col-span-12 lg:col-span-4 p-6"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Connection</div><div className={`mt-3 font-display text-[36px] font-semibold ${V3_TEXT.strong}`}>{connectionLabel}</div><p className={`mt-3 text-[13px] ${V3_TEXT.body}`}>{config.sourceHelp}</p></aside></div>{SourcesDetails ? <SourcesDetails health={health} /> : null}</> : null}
      </div>
    </ProductShell>
  );
}
