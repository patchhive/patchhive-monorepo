...
import { DashboardControls, ProgressiveList, useSavedDashboardViews } from "./workspace.jsx";
  ...
  return (
    <ProductShell productKey={config.productKey}>
      ...
      <div className="mx-auto max-w-[1440px] px-3 sm:px-6 pt-6 sm:pt-10 pb-24">
        ...
        {activeTab === "history" ? <section className="surface p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Saved evidence</div><h1 className={`font-display mt-2 text-[42px] font-semibold ${V3_TEXT.strong}`}>Run history.</h1><div className={`mt-1 text-[12px] ${V3_TEXT.mute}`}>{filteredHistory.length} in view / {history.length} saved</div></div><button className={`surface-inset h-9 rounded-full px-4 text-[11px] ${V3_TEXT.body}`} disabled={loading} onClick={() => refresh(false)} type="button">{loading ? "Refreshing…" : "Refresh history"}</button></div>{config.historyDashboard ? <div className="mt-6"><DashboardControls filters={historyFilters} onApplySavedView={historyDashboard.applyView} onDeleteSavedView={historyDashboard.deleteView} onFilterChange={historyDashboard.updateView} onQueryChange={setHistoryQuery} onReset={() => { historyDashboard.resetView(); setHistoryQuery(""); }} onSaveView={historyDashboard.saveView} onSortChange={(nextSort) => historyDashboard.updateView("sort", nextSort)} query={historyQuery} savedViews={historyDashboard.savedViews} searchPlaceholder={config.historyDashboard.searchPlaceholder || "Search run, repository, PR…"} sort={historyDashboard.view.sort} sortOptions={config.historyDashboard.sortOptions} /></div> : null}<div className="mt-7"><ProgressiveList empty={<div className={`py-14 text-center text-[13px] ${V3_TEXT.mute}`}>No saved runs match this view.</div>} initialCount={config.historyDashboard?.initialCount || 6} itemLabel="runs" items={filteredHistory} renderItem={(entry) => <HistoryRow config={config} entry={entry} key={entry.id} onLoad={load} />} /></div></section> : null}
