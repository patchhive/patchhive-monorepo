  ...
  return (
    <ProductShell productKey={config.productKey}>
      ...
      <div className="mx-auto max-w-[1440px] px-3 sm:px-6 pt-6 sm:pt-10 pb-24">
        ...
        {activeTab === "checks" ? <><div className="grid grid-cols-12 items-start gap-6"><section className="surface col-span-12 lg:col-span-8 p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Startup evidence</div><h1 className={`font-display mt-2 text-[42px] font-semibold ${V3_TEXT.strong}`}>System checks.</h1></div><button className={`surface-inset h-9 rounded-full px-4 text-[11px] ${V3_TEXT.body}`} disabled={loading} onClick={() => refresh(false)} type="button">{loading ? "Refreshing…" : "Refresh checks"}</button></div><div className="mt-7"><StartupCheckList checks={checks} /></div></section><aside className="surface col-span-12 lg:col-span-4 p-6"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Unified backend</div><div className={`mt-3 font-display text-[46px] font-semibold ${V3_TEXT.strong}`}>{health.status || "unknown"}</div><dl className="mt-6 space-y-3"><SideValue label="Database" value={health.db_ok ? "ready" : "unavailable"} /><SideValue label={connectionName} value={connectionValue} /><SideValue label="Runs" value={String(history.length)} /></dl></aside></div>{ChecksDetails ? <ChecksDetails checks={checks} health={health} history={history} /> : null}</> : null}
