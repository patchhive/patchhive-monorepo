...
export function MemoryLibraryPanel({ apiBase, fetcher, form, onError, onRefresh }) {
  ...
  const repoMemoryCount = repo ? memories.filter((entry) => entry.repo === repo).length : memories.length;
  const memoryFiltersActive = Boolean(query.trim()) || dashboard.view.kind !== "all" || dashboard.view.disposition !== "all";
  const memoryScopeLabel = repo
    ? memoryFiltersActive
      ? `Filtered from ${repoMemoryCount} in selected repository · ${memories.length} across PatchHive`
      : `Selected repository · ${memories.length} across PatchHive`
    : memoryFiltersActive
      ? `Filtered from ${memories.length} across PatchHive`
      : "Across PatchHive";
  const filters = [
