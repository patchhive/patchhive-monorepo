...
function reviewFrom(candidate) {
  const interpretation = candidate.interpretation?.status === "observed" ? candidate.interpretation : null;
  return { title: candidate.title || "", outcome: candidate.outcome || "", lesson: interpretation?.proposed_lesson || candidate.lesson || "", prevention: interpretation?.proposed_prevention || candidate.prevention || "", affected_paths: (candidate.affected_paths || []).join("\n"), evidence: (candidate.evidence || []).join("\n"), disposition: "policy", pinned: true };
}

function interpretationLabel(candidate) {
  const interpretation = candidate.interpretation;
  if (!interpretation) return "AI unknown";
  if (interpretation.status === "observed") return `AI ${String(interpretation.classification || "unknown").replaceAll("_", " ")}`;
  if (interpretation.status === "failed") return `AI failed · ${String(interpretation.code || "unknown").replaceAll("_", " ")}`;
  if (interpretation.status === "not_observed") return `AI ${String(interpretation.reason || "not observed").replaceAll("_", " ")}`;
  return "AI unknown";
}
...
export default function FailGuardPanel({ apiBase, fetcher, form, onError, onRefresh }) {
  ...
  async function retryInterpretation() {
    ...
    setBusy(true); setMessage("");
    try {
      const data = await readJson(await fetcher(`${apiBase}/failguard/candidates/${encodeURIComponent(selected.id)}/interpret`, { method: "POST" }), "Could not retry FailGuard interpretation.");
      setMessage(data.message); setSelected(data.candidate); setReview(reviewFrom(data.candidate)); await load();
    } catch (error) { onError(error.message || "RepoMemory could not interpret the candidate."); } finally { setBusy(false); }
  }
  ...
  return <div className="space-y-6"><section className="surface p-6 sm:p-8"><div className={`flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}><ShieldAlert size={12} /> Reviewable failure memory</div><h1 className={`mt-2 font-display text-[42px] font-semibold ${V3_TEXT.strong}`}>FailGuard.</h1><p className={`mt-3 max-w-3xl text-[13px] leading-relaxed ${V3_TEXT.body}`}>Queue painful outcomes, review the proposed lesson, and promote only the guardrails that deserve durable policy weight.</p>{message ? <div className={`surface-inset mt-5 rounded-xl p-3 text-[12px] ${V3_TEXT.body}`}>{message}</div> : null}</section>
    ...
    <GuidanceNotice label="Promotion boundary">Products submit bad outcomes automatically when configured. FailGuard correlates repeated candidates and may attach a bounded AI interpretation, but the model cannot promote, dismiss, dispatch, or widen scope. Only an operator-edited promotion creates durable memory and active TrustGate, RepoReaper, MergeKeeper, and ReleaseSentry suggestions. Every later match is recorded.</GuidanceNotice>
  </div>;
}
