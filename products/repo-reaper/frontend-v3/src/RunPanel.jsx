...
function reportMarkdown({ dry, stream, targetSelectionMode }) {
  ...
  if (stream.logs.length) {
    lines.push("", `## Evidence (${stream.logs.length} retained events)`, "");
    if (stream.droppedLogs) lines.push(`> ${stream.droppedLogs} earlier live events exceeded this browser's buffer. The complete record is this run's saved evidence in History.`, "");
    stream.logs.forEach((entry) => lines.push(`- ${entry.msg || JSON.stringify(entry)}`));
  }
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 px-3 py-6 sm:px-6">
    <section className="surface grid gap-6 p-6 lg:grid-cols-[1.6fr_0.9fr] lg:p-8">
      <div>
        ...
        <h1 className={`mt-3 max-w-4xl font-display text-[42px] font-semibold leading-[1.05] ${V3_TEXT.strong}`}>{stream.running ? phaseHeadline(stream.phase, dry) : dry ? "Find the work before touching it." : "Find, validate, and deliver bounded fixes."}</h1>
      ...
      <aside className="surface-inset rounded-2xl p-5">
        ...
        <div className={`mt-3 font-display text-[30px] font-semibold ${V3_TEXT.strong}`}>{stream.running ? livePhaseLabel : priority ? "Candidate evidence" : noCandidates ? "No matching issues" : ready ? "Ready to scope" : "Squad setup needed"}</div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2"><Fact label="Candidates" value={items.length}/><Fact label="Cost" value={money(stream.runCost)}/><Fact label="Phase" value={livePhaseLabel}/></div>
      </aside>
    </section>
    ...
    <section className="surface p-6">
      ...
      <div className="mt-5"><DashboardControls filters={[{ key: "status", label: "Status", value: views.view.status, options: [{ value: "all", label: "All" }, ...[...new Set(items.map((item) => String(item.status || "queued")))].map((value) => ({ value, label: statusLabel(value) }))] }]} onApplySavedView={views.applyView} onDeleteSavedView={views.deleteView} onFilterChange={views.updateView} onQueryChange={(value) => views.updateView("query", value)} onReset={views.resetView} onSaveView={views.saveView} onSortChange={(value) => views.updateView("sort", value)} query={views.view.query} savedViews={views.savedViews} searchPlaceholder="Search repository, issue, status, feedback…" sort={views.view.sort} sortOptions={[{ value: "score", label: "Highest score" }, { value: "status", label: "Status" }, { value: "repo", label: "Repository" }]}/></div>
      <div className="mt-5"><ProgressiveList initialCount={6} batchCount={24} itemLabel="candidates" items={filtered} empty={<div className={`surface-inset rounded-xl p-8 text-center text-[12px] ${V3_TEXT.mute}`}>No candidate evidence in this view.</div>} renderItem={(item, index) => <article className="surface-inset rounded-xl p-4" key={item.id || item.issue_url || index}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className={`font-display text-[17px] font-semibold ${V3_TEXT.strong}`}>{item.title || item.issue_title || "Candidate issue"}</h3><p className={`mt-1 text-[11px] ${V3_TEXT.mute}`}>{item.repo || "repository pending"}{item.issue_number ? ` · issue #${item.issue_number}` : ""}</p><p className={`mt-2 text-[12px] leading-relaxed ${V3_TEXT.body}`}>{item.eligibility_reason || reasonLabel(item.reason) || item.feedback || item.fixability_reason || "Candidate is waiting for the next pipeline decision."}</p></div><div className="flex shrink-0 gap-2"><Chip tone={statusTone(item.status)}>{statusLabel(item.status || "queued")}</Chip><Chip>{item.fixability_score ?? item.score ?? "Not scored"}{item.fixability_score != null || item.score != null ? " score" : ""}</Chip></div></div>{item.pr_url ? <a className="mt-3 inline-block text-[11px] underline" href={item.pr_url} rel="noreferrer" target="_blank">Open PR #{item.pr_number || ""}</a> : null}</article>}/></div>
    </section>
    ...
    <section className="surface p-6">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[0.8fr_1.2fr]"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">{agents.map((agent) => { const live = stream.agentStatuses[agent.id] || {}; return <div className="surface-inset rounded-xl p-3" key={agent.id}><div className="flex items-center justify-between gap-2"><span className={`font-display text-[14px] ${V3_TEXT.strong}`}>{agent.name}</span><Chip tone={statusTone(live.status || agent.status)}>{statusLabel(live.status || agent.status || "idle")}</Chip></div><div className={`mt-1 text-[10px] ${V3_TEXT.mute}`}>{roleLabel(agent.role)} · {agent.provider}/{agent.model}</div>{live.task ? <div className={`mt-2 text-[11px] ${V3_TEXT.body}`}>{live.task}</div> : null}</div>; })}</div><div>{stream.droppedLogs ? <div className={`surface-inset mb-3 rounded-xl p-3 text-[11px] ${V3_TEXT.mute}`}>{stream.droppedLogs} earlier live events exceeded this browser's {LIVE_LOG_LIMIT}-event buffer and are not shown here. The complete record is saved with the run in History.</div> : null}<ProgressiveList initialCount={8} batchCount={30} itemLabel="events" items={stream.logs} empty={<div className={`surface-inset rounded-xl p-8 text-center text-[12px] ${V3_TEXT.mute}`}>Live logs appear here after the operation starts.</div>} renderItem={(entry, index) => <div className="surface-inset rounded-xl p-3" key={`${entry.ts || "log"}-${index}`}><div className="flex items-start gap-3"><ClipboardCopy className={V3_TEXT.dim} size={13}/><div><div className={`text-[11px] leading-relaxed ${V3_TEXT.body}`}>{entry.msg || JSON.stringify(entry)}</div><div className={`mt-1 text-[9px] uppercase tracking-wider ${V3_TEXT.mute}`}>{entry.agent || roleLabel(entry.role) || "RepoReaper"}{entry.type === "error" ? " · error" : entry.type === "warn" ? " · warning" : ""}</div></div></div></div>}/></div></div>
