...
const PROVIDERS = ["codex", "openai", "anthropic", "gemini", "groq", "openrouter", "custom", "ollama"];
const DEFAULT_MODELS = { codex: "gpt-5.4", openai: "gpt-5.4-mini", anthropic: "claude-sonnet-4-6", gemini: "gemini-2.5-pro", groq: "llama-3.3-70b-versatile", openrouter: "openrouter/free", custom: "gpt-4.1-mini", ollama: "llama3.2" };
const OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1";
const PROVIDER_LABELS = { codex: "Codex (ChatGPT subscription)", openai: "OpenAI / Compatible", anthropic: "Anthropic", gemini: "Gemini", groq: "Groq", openrouter: "OpenRouter", custom: "Custom OpenAI-Compatible", ollama: "Ollama" };
...
function nextProviderState(current, provider) {
  if (provider === "codex") {
    return { ...current, provider, model: DEFAULT_MODELS.codex, base_url: "", api_key: "" };
  }
...
function Input({ disabled = false, label, onChange, placeholder, type = "text", value }) {
  return <label className="block"><span className={`text-[9px] uppercase tracking-[0.18em] ${V3_TEXT.mute}`}>{label}</span><input className={`surface mt-1.5 h-10 w-full rounded-xl bg-transparent px-3 text-[12px] outline-none disabled:cursor-not-allowed disabled:opacity-55 ${V3_TEXT.strong}`} disabled={disabled} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} type={type} value={value || ""}/></label>;
}
...
export default function SquadPanel({ agents, apiBase, authToken, config, cooldowns, fetcher, onError, onRefresh, presets }) {
  ...
  function applyDefaults() {
    setDraft((current) => current.map((agent) => ({
      ...agent,
      provider: defaults.provider,
      model: defaults.model || DEFAULT_MODELS[defaults.provider] || agent.model,
      ...(defaults.provider === "codex" ? { base_url: "", api_key: "", api_key_set: false } : {}),
      ...(defaults.provider !== "codex" && defaults.base_url ? { base_url: defaults.base_url } : {}),
      ...(defaults.provider !== "codex" && defaults.api_key ? { api_key: defaults.api_key } : {}),
      ...(defaults.bot_token ? { bot_token: defaults.bot_token } : {}),
      ...(defaults.bot_user ? { bot_user: defaults.bot_user } : {}),
    })));
  }

  const missingRoles = ROLES.filter((role) => !draft.some((agent) => agent.role === role));
  const codexAuth = config?.AI_LOCAL_STATUS?.providers?.codex?.auth;
    ...
    <section className="grid gap-6 lg:grid-cols-2">
      <article className="surface p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          <Input disabled={defaults.provider === "codex"} label="Base URL" onChange={(value) => setDefaults((current) => ({ ...current, base_url: value }))} placeholder={defaults.provider === "codex" ? "Managed by PatchHive Local AI" : "OpenAI-compatible endpoint"} value={defaults.base_url}/>
          <Input disabled={defaults.provider === "codex"} label="Provider API key" onChange={(value) => setDefaults((current) => ({ ...current, api_key: value }))} placeholder={defaults.provider === "codex" ? "Codex owns the login" : "Leave blank to preserve/fallback"} type="password" value={defaults.api_key}/>
        ...
        <div className={`mt-4 space-y-1 text-[11px] ${V3_TEXT.mute}`}>
          {defaults.provider === "codex" ? <div>{codexAuth?.status === "authenticated" ? "Codex is authenticated with your ChatGPT subscription. RepoReaper receives no OAuth tokens." : `Codex subscription is not ready (${codexAuth?.status || "gateway not observed"}). Sign in through the Codex-owned local AI login.`}</div> : null}
      ...
      <article className="surface p-6">
        ...
        <GuidanceNotice label="Secret storage">Browser responses expose only configured flags. Provider keys and bot-token overrides stay encrypted in SQLite when a stable RepoReaper or suite encryption key is configured. Codex subscription agents never accept or store a provider key or custom base URL.</GuidanceNotice>
      </article>
    </section>

    <section className="surface p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className={`flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}><Bot size={12}/> Active team draft</div><h2 className={`mt-2 font-display text-[28px] font-semibold ${V3_TEXT.strong}`}>{draft.length} configured agents</h2></div><div className="flex flex-wrap gap-2"><button className="surface-inset h-9 rounded-full px-4 text-[11px]" disabled={working} onClick={() => setDraft(ROLES.map(blankAgent))} type="button">Recruit starter team</button><button className="surface-inset h-9 rounded-full px-4 text-[11px]" disabled={working} onClick={() => setDraft((current) => [...current, blankAgent(missingRoles[0] || "reaper")])} type="button">Add agent</button><button className="h-9 rounded-full px-4 text-[11px] font-semibold text-white disabled:opacity-40" disabled={working || !draft.length} onClick={() => perform(() => saveTeam(), "Could not save squad")} style={{ backgroundImage: "linear-gradient(90deg, var(--accent), var(--accent-2))" }} type="button">{working ? "Saving…" : "Save active squad"}</button></div></div><div className="mt-5"><ProgressiveList initialCount={6} batchCount={20} itemLabel="agents" items={draft} empty={<div className={`surface-inset rounded-xl p-10 text-center text-[12px] ${V3_TEXT.mute}`}>No agents configured.</div>} renderItem={(agent) => <article className="surface-inset rounded-xl p-4" key={agent.id}><div className="flex flex-col gap-4 lg:flex-row lg:items-start"><div className="grid flex-1 gap-3 sm:grid-cols-2 lg:grid-cols-4"><Input label="Name" onChange={(value) => updateAgent(agent.id, "name", value)} value={agent.name}/><label><span className={`text-[9px] uppercase tracking-wider ${V3_TEXT.mute}`}>Role</span><select className={`surface mt-1.5 h-10 w-full rounded-xl bg-transparent px-3 text-[12px] ${V3_TEXT.strong}`} value={agent.role} onChange={(event) => updateAgent(agent.id, "role", event.target.value)}>{ROLES.map((role) => <option key={role} value={role}>{roleLabel(role)}</option>)}</select></label><label><span className={`text-[9px] uppercase tracking-wider ${V3_TEXT.mute}`}>Provider</span><select className={`surface mt-1.5 h-10 w-full rounded-xl bg-transparent px-3 text-[12px] ${V3_TEXT.strong}`} value={agent.provider} onChange={(event) => updateAgent(agent.id, "provider", event.target.value)}>{PROVIDERS.map((provider) => <option key={provider} value={provider}>{PROVIDER_LABELS[provider] || provider}</option>)}</select></label><Input label="Model" onChange={(value) => updateAgent(agent.id, "model", value)} value={agent.model}/><Input disabled={agent.provider === "codex"} label="Base URL" onChange={(value) => updateAgent(agent.id, "base_url", value)} placeholder={agent.provider === "codex" ? "Managed by PatchHive Local AI" : "Optional override"} value={agent.base_url}/><Input disabled={agent.provider === "codex"} label={`Provider key${agent.api_key_set ? " · configured" : ""}`} onChange={(value) => updateAgent(agent.id, "api_key", value)} placeholder={agent.provider === "codex" ? "Codex owns the login" : agent.api_key_set ? "Leave blank to preserve" : "Optional override"} type="password" value={agent.api_key}/><Input label={`Bot token${agent.bot_token_set ? " · configured" : ""}`} onChange={(value) => updateAgent(agent.id, "bot_token", value)} placeholder={agent.bot_token_set ? "Leave blank to preserve" : "Optional override"} type="password" value={agent.bot_token}/><Input label="Bot user" onChange={(value) => updateAgent(agent.id, "bot_user", value)} value={agent.bot_user}/></div><div className="flex shrink-0 items-center gap-2"><Chip tone={statusTone(agent.status)}>{statusLabel(agent.status || "idle")}</Chip><button aria-label={`Remove ${agent.name}`} className="surface grid h-9 w-9 place-items-center rounded-full text-red-700 dark:text-red-300" onClick={() => setDraft((current) => current.filter((item) => item.id !== agent.id))} type="button"><Trash2 size={13}/></button></div></div></article>}/></div></section>
