    ...
    <div style={{ display: "grid", gap: 18 }}>
      <div style={{ ...S.panel, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ color: "var(--text-dim)", fontSize: 12 }}>
            DepTriage needs GitHub access and a healthy local DB before its update-now calls mean much.
          </div>
        </div>
      ...
      {health && (
        <div style={{ ...S.panel, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
          ...
          <Stat label="Tracked Items" value={health.tracked_item_count} />
          <Stat label="Update Now" value={health.update_now_count} color="var(--accent)" />
          <Stat label="Watch" value={health.watch_count} color="var(--gold)" />
          <Stat label="Ignore" value={health.ignore_count} color="var(--text-dim)" />
          <div>
