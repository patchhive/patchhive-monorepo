...
import { CalendarClock, KeyRound, ListChecks, LoaderCircle, Save, ShieldCheck } from "lucide-react";
...
export default function ControlsPanel({ apiBase, fetcher, form, health, onError, onLoad, onRefresh, serialize, setForm, toFormParams }) {
  ...
  return <div className="space-y-6">
    <section className="surface p-6 sm:p-8"><div className={`text-[10px] uppercase tracking-[0.22em] ${V3_TEXT.mute}`}>Scan operations</div><h1 className={`mt-2 font-display text-[42px] font-semibold ${V3_TEXT.strong}`}>Presets, schedules, and scope.</h1><p className={`mt-3 max-w-3xl text-[13px] leading-relaxed ${V3_TEXT.body}`}>Reuse direct targets or broad discovery scopes, schedule read-only scans, and enforce repository boundaries before SignalHive reads GitHub.</p>{message ? <div className={`surface-inset mt-5 rounded-xl p-3 text-[12px] ${V3_TEXT.body}`}>{message}</div> : null}</section>
    ...
    <section className="surface p-6"><PanelTitle icon={ShieldCheck} subtitle="These controls change local SignalHive configuration only. They do not write to GitHub.">Safety boundary</PanelTitle><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className={`surface-inset rounded-xl p-4 text-[11px] leading-relaxed ${V3_TEXT.mute}`}><span className={`block font-display text-[15px] ${V3_TEXT.strong}`}>Read only</span>Direct and discovery schedules gather evidence; they do not create issues, comments, commits, or pull requests.</div><div className={`surface-inset rounded-xl p-4 text-[11px] leading-relaxed ${V3_TEXT.mute}`}><span className={`block font-display text-[15px] ${V3_TEXT.strong}`}>Explicit exclusion</span>Repository opt-outs and denylists are checked before either scan mode reads a target.</div><div className={`surface-inset rounded-xl p-4 text-[11px] leading-relaxed ${V3_TEXT.mute}`}><span className={`block font-display text-[15px] ${V3_TEXT.strong}`}>Bounded output</span>Discovery scopes enforce repository and minimum-star limits. Both scan modes enforce issue and stale-window limits.</div></div></section>
  </div>;
}
