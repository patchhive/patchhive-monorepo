...
function WorkspaceDetails({ fetcher, health, onError, result }) {
  ...
  const comparableRepos = trend ? Math.max(Number(trend.comparable_repos || 0), Number(trend.rising_repos || 0) + Number(trend.improving_repos || 0) + Number(trend.steady_repos || 0)) : 0;
  const cohortComparable = trend ? Boolean(trend.cohort_comparable) || (comparableRepos > 0 && comparableRepos * 2 >= Math.max(comparableRepos + Number(trend.new_repos || 0), comparableRepos + Number(trend.dropped_repos || 0))) : false;
  ...
  mapItem: (repo) => {
    return {
      ...
      facts: [
        ...
        { label: "TODO / FIXME matches", value: `${repo.todo_available ? repo.todo_count || 0 : "unavailable"} / ${repo.fixme_available ? repo.fixme_count || 0 : "unavailable"}` },
      ],
