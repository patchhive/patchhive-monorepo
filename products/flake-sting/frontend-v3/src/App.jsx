...
import { countLabel, IntegratedProductApp, ProductLoginScreen, ProductShell, V3_TEXT } from "@patchhivehq/ui-v3";
...
const config = {
  ...
  metrics: (result, overview, health) => { const m = result?.metrics || {}; const c = overview?.counts || {}; return result ? [
    ...
    { label: "Failed runs", value: m.failed_runs || 0, footerLeft: "workflows", footerRight: countLabel(m.workflow_runs, "run"), tone: "from-amber-600/70 to-yellow-800/50" },
    ...
    { label: "Scans", value: c.scans || health.scan_count || 0, footerLeft: "saved", footerRight: countLabel(c.repos, "repo"), tone: "from-slate-500/70 to-slate-800/60" },
  ...
  hero: (result) => result ? { lead: countLabel(result.metrics?.flaky_signals, "signal"), middle: "need", highlight: "attention." } : { lead: "Unstable checks", middle: "need", highlight: "evidence." },
...
export default function App() { const auth = useApiKeyAuth({ apiBase: API, storageKey: "flake-sting_api_key" }); const fetcher = useMemo(() => createApiFetcher(auth.apiKey), [auth.apiKey]); if (!auth.checked) return <ProductShell productKey={config.productKey}><div className={`min-h-screen grid place-items-center ${V3_TEXT.mute}`}>Connecting…</div></ProductShell>; if (auth.needsAuth) return <ProductLoginScreen apiBase={API} auth={auth} config={config} />; return <IntegratedProductApp apiBase={API} auth={auth} config={config} fetcher={fetcher} />; }
