...
function reportMarkdown(result) {
  ...
  const lines = [
    ...
    "",
    result?.summary || "TrustGate diff-policy review.",
    "",
...
const config = {
  ...
  fields: (_health, form) => [
    ...
    ...(form.source_mode === "manual" ? [
      ...
      { key: "ai_source", label: "AI source", placeholder: "Codex, Copilot, human…" },
      { key: "publish_status", label: "Publish TrustGate report", help: "Attempts GitHub check/status output and a maintained PR comment for this run.", type: "checkbox" },
    ]),
  ],
  validate: (form) => {
  ...
  hero: (result) => result ? { lead: result.github?.pr_number ? `PR #${result.github.pr_number}` : "This diff", middle: "is", highlight: `${result.recommendation || "pending"}.` } : { lead: "Every diff", middle: "needs a", highlight: "trust call." },
  status: (result, overview) => ({ label: result?.recommendation || "—", detail: result?.summary || "Review a pasted diff or live pull request to begin", progress: result ? `${Math.max(4, result.risk_score || 0)}%` : "8%", stats: [["Risk", result?.risk_score || 0], ["Findings", result?.findings?.length || 0], ["Runs", overview?.counts?.reviews || 0]] }),
  ...
  targetSubtitle: (result) => result?.github?.pr_number ? `Pull request #${result.github.pr_number} · ${result.recommendation}` : result ? `${result.source_kind} · ${result.recommendation}` : "Diff policy review",
  ...
  historyBadges: (entry) => [{ label: entry.recommendation || "saved", tone: decisionTone(entry.recommendation) }, { label: `${entry.risk_score || 0} risk`, tone: entry.risk_score >= 70 ? "hot" : entry.risk_score >= 35 ? "warn" : "neutral" }],
