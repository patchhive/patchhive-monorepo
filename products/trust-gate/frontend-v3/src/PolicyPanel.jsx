...
function Field({ label, onChange, rows, type = "text", value }) {
  return <label className="block"><span className={`text-[9px] uppercase tracking-[0.18em] ${V3_TEXT.mute}`}>{label}</span><div className="surface-inset mt-2 rounded-xl px-3 py-2">{rows ? <textarea className={`w-full resize-y bg-transparent font-mono text-[11px] leading-relaxed outline-none ${V3_TEXT.strong}`} onChange={(event) => onChange(event.target.value)} rows={rows} value={value} /> : <input className={`h-8 w-full bg-transparent text-[12px] outline-none ${V3_TEXT.strong}`} onChange={(event) => onChange(event.target.value)} type={type} value={value} />}</div></label>;
}
